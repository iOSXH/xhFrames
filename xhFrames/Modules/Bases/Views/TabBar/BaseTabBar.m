...
- (void)initSubViews{
    ...
    for (NSInteger i = 0; i < tabConfigs.count; i++) {
        ...
        BaseTabBtn *btn = [[BaseTabBtn alloc] initWithFrame:CGRectMake(startX, 0, width, height) config:config];
...
- (void)setSelectedIndex:(NSInteger)selectedIndex{
    ...
    BaseTabBtn *btn = [self.tabButtons objectOrNilAtIndex:selectedIndex];
    if (!btn) {
        return;
    }
...
- (void)resetBadge:(BOOL)flag atIndex:(NSInteger)index{
    BaseTabBtn *btn = [self.tabButtons objectAtIndex:index];
    [btn resetBadge:flag];
}
